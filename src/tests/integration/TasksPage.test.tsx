...
describe('TasksPage - integración completa', () => {
  ...
  it('debe cargar y mostrar las tareas del mock', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Preparar presentación trimestral')).toBeInTheDocument();
    }, { timeout: 5000 });

    expect(screen.getByText('Comprar víveres para la semana')).toBeInTheDocument();
  });
  ...
  it('debe filtrar tareas al escribir en la barra de búsqueda', async () => {
    ...
    await userEvent.type(searchInput, 'impuestos');

    await waitFor(() => {
      expect(screen.getByText('Entregar informe de impuestos')).toBeInTheDocument();
      expect(screen.queryByText('Comprar víveres para la semana')).not.toBeInTheDocument();
    });
  });
