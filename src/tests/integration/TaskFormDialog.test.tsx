...
describe('TaskFormDialog', () => {
  ...
  describe('sección IA', () => {
    ...
    it('debe generar subtareas al hacer click en "Sugerir subtareas"', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Generadas por IA/)).toBeInTheDocument();
      }, { timeout: 3000 });
    });
    ...
    it('debe clasificar la tarea al hacer click en "Auto-clasificar"', async () => {
      ...
      await userEvent.click(screen.getByRole('button', { name: /auto-clasificar/i }));

      await waitFor(() => {
        // Verifica que el select de categoría cambió a "Trabajo"
        const categorySelect = screen.getByLabelText('Categoría');
        expect(categorySelect).toHaveTextContent('Trabajo');
      }, { timeout: 3000 });
    });
  });
